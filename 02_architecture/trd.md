...
### 3.2 Multimodal Relational Ingestion Engine
- **OpenCV Line Skeletonization**: Morphological thinning & Hough transforms extract vector coordinates and junction paths from P&IDs.
- **Tiled VLM Patching**: Slices high-res prints into sub-grids; VLM extracts equipment tags (e.g., `10-P-101-CS`) and valve symbols.
- **Geometric Snapping**: Maps symbol centroids to vector line endpoints via adjacency matrices to build NetworkX graphs.
